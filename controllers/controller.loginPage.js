...
exports.postLogin = (req, res) => {
    ...
    db.query('SELECT * FROM clientProfile WHERE clientProfile.email = ?', [email], (err, results) => {
        if (err) {
            console.log(err);
        ...
        if (results.length == 0) {
            db.query('SELECT * FROM freelancerProfile WHERE freelancerProfile.email = ?', [email], (err, results) => {
                if (err) {
                    console.log(err);
                    return res.status(500).render('errorPage', {
                        msg: 'An error occurred while querying the database',
